...
def parseEpisode(filename):
    ...
    season = 0
    episode = 0
    match = re.search('s([0-9]+)[\W]?e([0-9]+)',repr(filename).lower())
    if match:
        season = int(match.group(1))
        episode = int(match.group(2))
        # Next look for NN*MM
    else:
        match = re.search('([0-9]+)[a-z]([0-9]+)',repr(filename).lower())
        if match:
            # Next look for NNMM
            season = int(match.group(1))
            episode = int(match.group(2))
        else:
            match = re.search("[ .]([0-9]{3,4})[ .]",repr(filename).lower())
            if match:
